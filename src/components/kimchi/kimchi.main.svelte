...
{#if loaded}
	<Sound
		bind:chapter={currentChapter}
		bind:eventClicked
		bind:mode={cutSceneMode}
		bind:soundon
	/>
	<div
		class="scene"
		bind:this={scene}
		bind:clientWidth={insideWidth}
		...
		{#if currentChapter == 0}
			{#key currentChapter}
				<Intro
					chapter="0"
					bind:eventClicked
					bind:chapterTracker={currentChapter}
					w={windowWidth}
					h={sceneHeight}
					{maxWidth}
					bind:soundon
		...
		{:else if [1, 4, 7, 10, 13].includes(currentChapter)}
			{#key currentChapter}
				<PreScene
					bind:chapter={currentChapter}
					bind:eventClicked
					cutsceneText={copy["scene" + currentChapter]}
					bind:chapterTracker={currentChapter}
					w={windowWidth}
					h={sceneHeight}
					{maxWidth}
		...
		{:else if [2, 5, 8, 11].includes(currentChapter)}
			{#key currentChapter}
				<Scene
					...
					bind:eventClicked
					hoverHints={copy["scene" + currentChapter]}
					bind:chapterTracker={currentChapter}
					bind:scrolled
					bind:scrolledY
					bind:scrollAmount
	...
	<Progress bind:eventClicked bind:chapter={currentChapter} />
{/if}
